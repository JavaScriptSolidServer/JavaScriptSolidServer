...
program
  .name("jss")
  .description(
    "JavaScript Solid Server - A minimal, fast, JSON-LD native Solid server",
  )
  .version(pkg.version);
...
program
  ...
  .option(
    "--base-domain <domain>",
    'Base domain for subdomain pods (e.g., "example.com")',
  )
  .option(
    "--mashlib",
    "Enable Mashlib data browser (local mode, requires mashlib in node_modules)",
  )
  .option(
    "--mashlib-cdn",
    "Enable Mashlib data browser (CDN mode, no local files needed)",
  )
  .option("--no-mashlib", "Disable Mashlib data browser")
  .option(
    "--mashlib-version <version>",
    "Mashlib version for CDN mode (default: 2.0.0)",
  )
  .option(
    "--solidos-ui",
    "Enable modern Nextcloud-style UI (requires --mashlib)",
  )
  ...
  .option(
    "--nostr-max-events <n>",
    "Max events in relay memory (default: 1000)",
    parseInt,
  )
  ...
  .option(
    ...
    "Username for single-user mode (default: me)",
  )
  ...
  .action(async (options) => {
    try {
      // ADD VALIDATION CLI FOR SINGLE USERNAME
      if (options.singleUserName && options.singleUserName.startsWith("--")) {
        console.error(
          `Error: --single-user-name value "${options.singleUserName}" looks like a flag.`,
        );
        console.error(
          "Did you forget to provide a username? Example: --single-user-name melvin",
        );
        process.exit(1);
      }
      ...
      if (!config.quiet) {
        ...
        if (config.subdomains)
          console.log(
            `  Subdomains: ${config.baseDomain} (XSS protection enabled)`,
          );
        ...
        if (config.solidosUi)
          console.log("  SolidOS UI: enabled (modern interface)");
        if (config.git) console.log("  Git: enabled (clone/push support)");
        if (config.nostr) console.log(`  Nostr: enabled (${config.nostrPath})`);
        if (config.activitypub)
          console.log(`  ActivityPub: enabled (@${config.apUsername || "me"})`);
        if (config.singleUser)
          console.log(
            `  Single-user: ${config.singleUserName || "me"} (registration disabled)`,
          );
        else if (config.inviteOnly) console.log("  Registration: invite-only");
        if (config.webidTls)
          console.log("  WebID-TLS: enabled (client certificate auth)");
  ...
  .action(async (options) => {
    ...
    if (options.yes) {
      ...
      config = {
        ...
        conneg: await confirm(
          "Enable content negotiation (Turtle support)?",
          defaults.conneg,
        ),
...
quotaCmd
  ...
  .action(async (username, size, options) => {
    try {
      ...
      console.log(
        `Current usage: ${formatBytes(quota.used)} (${Math.round((quota.used / quota.limit) * 100)}%)\n`,
      );
    } catch (err) {
      console.error(`Error: ${err.message}`);
      process.exit(1);
  ...
  .action(async (username, options) => {
    try {
      ...
      if (quota.limit === 0) {
        ...
        console.log(`  Limit: ${formatBytes(quota.limit)}`);
        console.log(
          `  Usage: ${Math.round((quota.used / quota.limit) * 100)}%\n`,
        );
      }
    } catch (err) {
      console.error(`Error: ${err.message}`);
